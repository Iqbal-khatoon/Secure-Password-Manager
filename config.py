...
def make():
	if checkConfig():
		...
		logging.info(" Configuration already exists")
		return

	printc("[green][+] Creating new config [/green]")
	logging.info(" New Configuration has been done")
	...
	logging.info(" Configuation is Done and master pasword has been created")
...
def delete():
	...
	logging.info("Old Configuation has been deleted means all entries in the DB no longer exists")


	if not checkConfig():
		printc("[yellow][-][/yellow] No configuration exists to delete!")
		logging.info("No configuration exists to delete!")
		return
...
def remake():
	...
	logging.info("Remaking the Configuration")
	delete()
	make()
